# ARCHON Codebase Ingestion & Dataset Preparation Tools
...
* **`codebase_pipeline.py`**: Ingests local directories and remote Git repositories into structured `.jsonl` or Gzip-compressed `.jsonl.gz` datasets. It handles lossless text minification and streams output directly to standard LangChain `Document` objects.
* **`split_dataset.py`**: Partitions large dataset files (`.icb`, `.jsonl`, `.json`, `.csv`, `.md`, `.txt`) into target byte-size limits without breaking structural lines, CSV headers, or JSON syntax boundaries.
...
* **Optional Package**: `langchain-core` (if omitted, `codebase_pipeline.py` automatically uses an internal compatibility fallback stub).

---

## How to Use `codebase_pipeline.py`
